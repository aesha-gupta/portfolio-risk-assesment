...
from data.coindcx_api import CoinDCXFetcher
...
st.sidebar.markdown("---")
st.sidebar.subheader("🔗 Import from CoinDCX")
st.sidebar.markdown("Sync your real trades securely via the API. We aggregate your partial fills, compute your true average buy price, and auto-translate to Yahoo Finance tickers.")
coindcx_key = st.sidebar.text_input("API Key", type="password")
coindcx_secret = st.sidebar.text_input("API Secret", type="password")

if st.sidebar.button("Sync My Trades"):
    if not coindcx_key or not coindcx_secret:
        st.sidebar.error("Please enter both Key and Secret.")
    else:
        with st.sidebar.status("Connecting to CoinDCX API...", expanded=True) as status:
            fetcher_cx = CoinDCXFetcher(api_key=coindcx_key, api_secret=coindcx_secret)
            try:
                st.write("Authenticating HMAC Request...")
                raw_trades = fetcher_cx.fetch_raw_trades()
                st.write(f"Fetched {len(raw_trades)} raw trades from CoinDCX.")
            except Exception as e:
                st.write(f"API Authentication Failed. Auto-loading realistic Hackathon Mock Ledger to demonstrate pipeline capabilities...")
                # MOCK DATA FOR DEMO PURPOSES
                raw_trades = [
                    {"symbol": "BTCINR", "side": "buy", "price": "3500000", "quantity": "0.1", "timestamp": 1673740800000},
                    {"symbol": "BTCINR", "side": "buy", "price": "3600000", "quantity": "0.15", "timestamp": 1674740800000},
                    {"symbol": "BTCINR", "side": "sell", "price": "3700000", "quantity": "0.05", "timestamp": 1675740800000},
                    {"symbol": "ETHUSDT", "side": "buy", "price": "1800", "quantity": "2.0", "timestamp": 1676740800000},
                    {"symbol": "ETHUSDT", "side": "buy", "price": "1700", "quantity": "1.0", "timestamp": 1677740800000},
                    {"symbol": "SOLINR", "side": "buy", "price": "1900", "quantity": "40.0", "timestamp": 1678740800000},
                ]
            
            st.write("Aggregating partial fills, computing true buy prices, and mapping to Yahoo Finance engine...")
            df_clean = fetcher_cx.process_trade_history(raw_trades)
            
            if not df_clean.empty:
                # Keep Ticker, Quantity, Date
                df_clean = df_clean[["Ticker", "Quantity", "Date"]]
                st.session_state.portfolio_df = df_clean
                status.update(label="Sync Complete! Portfolio automatically populated.", state="complete")
                st.rerun() # Refresh the dataframe editor dynamically!
            else:
                st.error("No valid buy trades found.")
